...
| `spec_migration` | `"rewrite"` | `/jim:partition rename` / `split` — identity-on-move preference for a moved numbered spec's recorded group identity: `rewrite` edits it to the new group (substance untouched; ambiguous prose frozen on doubt), `forward` freezes the bodies behind the ledger `op=rename` / `op=split` alias, `immutable` leaves the source in place — split-native (a rename, which relocates the group's home, runs it as `forward`); an unrecognized value degrades to `rewrite`. Governs numbered specs 001+; the `000-blueprint` re-identifies in every mode |
