...
| **Spec-driven frameworks** (GitHub Spec Kit, OpenSpec, Amazon Kiro, cc-sdd) | Real spec→plan→tasks→implement pipelines; testable-AC grammars (EARS / WHEN-THEN); cross-artifact consistency linting; often agent-agnostic across many coding agents | Template/generation-centric; thin on collaborative interview, codebase research, and design-time security; several optimize for autonomy over human-in-loop transparency; no strategic vision/roadmap layer; no living current-state spec with a build→review fold-back loop, no cross-group contract graph, and no code-conformance engine (even the mid-2026 `/converge` / reconcile additions only append build tasks against ephemeral per-feature artifacts) |
| **Skill collections** (obra/superpowers, Matt Pocock, Addy Osmani) | High-craft individual skills (TDD, brainstorming, planning, review) with strong meta-authoring conventions; very large adoption | À-la-carte skills, not an integrated lifecycle; no compounding spec/research/plan archive; no strategic-alignment gating; review/verification skills exist but aren't tied to a living spec or a contract surface |
| **Architecture-conformance, contract & delivery-telemetry tools** (ArchUnit, SonarQube Architecture Management, Pact/BDCT, Context Mapper, Nx boundaries, Liza, DORA platforms) | Mature, deterministic, free-per-run enforcement of module boundaries, cross-service contracts, architectural drift, and delivery metrics; battle-tested at scale | Each solves one slice in isolation; not integrated into a spec→build lifecycle; mostly deterministic-only with no design-time authorial intent; operate at runtime / post-hoc or as external dashboards; no collaborative spec/research/plan lineage |
...
*Competitive landscape refreshed 2026-07-24 — see `docs/research/20260724-competitive-landscape-sdd-skills.md` for the full survey, per-capability study anchors, and gap analysis.*

**Differentiation:** Jim is a convention-over-configuration agentic SDLC native to Claude Code, with specialized agents that maintain domain context and living documents that support agile iteration. Two pillars now define its edge. First, the *integrated, human-in-the-loop* path — collaborative spec interview, grounded research, design-time security, and strategic alignment — keeping the developer in control at every gate. Second, its institutional memory is now *executable*: a living, present-tense blueprint whose provides/requires faces reconcile into a contract graph, verified against the code and folded back from every build and review. Where the ecosystem ships boundary-enforcement, contract-testing, drift-detection, and delivery-telemetry as separate deterministic tools, jim integrates them into one transparent pipeline — and where competitors optimize for autonomy and template generation, jim compounds a verifiable architectural record instead.
...
- **Not a project management tool.** Jim does not replace Jira, Linear, or any ticketing system, and does not manage team workflows. Recording who filed a discovery and who currently holds it is in scope, as is querying and rolling up that record — claiming is pull-based and self-service, so the collection stays a shared account of what was found and who picked it up. What stays out is the workflow layer around it: assigning work to someone else, notification, capacity planning, sprints, boards, and due dates.
