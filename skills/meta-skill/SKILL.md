...
Use `$ARGUMENTS` as a hint for the skill name. Search `!`bash ${CLAUDE_PLUGIN_ROOT}/skills/conf/scripts/jimconf.sh get specs`/jim/` for a matching approved spec (`status: approved` in frontmatter), or ask the user which spec to build from.
