...
- **`lean` depth:** skip the broad fan-out — investigate only security-relevant regions and assess the rest from the diff spine yourself. This is a *chosen* depth, not a degradation: it is recorded as the depth used, and the next bullet does not apply to it.
- **A fan-out that did not run is a degradation, not a depth.** `lean` and the cap are the only two reasons the high-stakes set goes uninvestigated with the review still whole. If the fan-out did not run for any *other* reason — the Agent tool is unavailable, or a standing session directive withheld it — then the deep investigation **did not happen**, and your own read of the same diff is not a substitute for it: that substitution is precisely what this review is built to avoid, since an author reviewing their own range is the comparison that produced `minor-drift`/5 findings where a fan-out over the identical commits produced `major-drift`/15. Name it in `review.md` (Step 8) as an undelegated fan-out, list the regions and ACs that consequently rest on spine-level reading only, and count it on the ledger (`undelegated=<n>`). The verdict is still yours to assign — but it is assigned over thinner evidence, and the artifact must say so.
- **Followability** (VISION — *not a black box*): before spawning, state which targets you are investigating and at what depth; after, note how many investigators ran. When those two numbers differ for any reason other than the cap, the difference is the undelegated set — reconcile them explicitly rather than letting the second number go unstated.
...
```
bash ${CLAUDE_PLUGIN_ROOT}/skills/ledger/scripts/jimledger.sh event <spec-dir> review finished alignment=<verdict> findings=<n> undelegated=<n>
```

`<verdict>` is the assigned verdict; `<n>` is your findings count. `undelegated=<n>` counts the high-stakes targets whose investigator was never dispatched (4c) — `0` on a whole run, recorded **always** rather than only when non-zero, so a `0` distinguishes a fanned-out review from a record that predates the counter. `review.md`, not the ledger, is the **authoritative** verdict — the ledger line is an advisory record so the verdict trajectory survives re-runs (`major-drift → aligned` stays visible). Skip silently if `jimledger.sh` is absent.
...
FOR each candidate (1-based row_index `i`):
  ...
  - File it: `bash ${CLAUDE_PLUGIN_ROOT}/skills/issue/scripts/new.sh --auto --title "<title>" --priority <p> --labels "<csv>" --origin "<origin>" --body-file "<tmp>"`. `--auto` declares the batch unreviewed, which is what lets the emitter apply the placement scrub gate (`skills/issue/SKILL.md` § 7a).
...
- [ ] The investigator fan-out was actually dispatched. If it was not — for any reason other than `lean` depth or the cap — `review.md` names the undelegated coverage and lists what consequently rests on spine-level reading only, and `undelegated=<n>` rode the ledger event. A review that reports full coverage over an investigation that never ran is the one output this skill must never produce.
...
- [ ] The review stage's boundaries were recorded on the ledger — `review started` after the spec.md precondition, `review finished alignment=… findings=… undelegated=…` after the verdict and **before** composing `review.md` — and `review.md`'s own metrics were re-read after `finished`.
