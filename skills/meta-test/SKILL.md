...
**Gate 1 — Spec:** Search the spec groups via !`bash ${CLAUDE_PLUGIN_ROOT}/skills/file/scripts/jimfile.sh glob specs` for a spec whose plan creates `skills/<feature>/scripts/<name>.sh`. The spec frontmatter must have `status: approved`. If no approved spec exists, spawn `@jim:pm` via the Agent tool to create one (or tell the user to run `/jim:spec` if pm is unavailable). Stop execution.
