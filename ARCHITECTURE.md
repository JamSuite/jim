...
| Issue Collection | Markdown files | `docs/issues/` (configurable via `issues_path`) | Discovery-artifact capture per spec 017; one file per issue (carrying a display-only `num` ordinal as of spec 019; filename prefix configurable via `issue_id_prefix` as of spec 021) plus auto-generated `INDEX.md` regenerated on every write | PM via `/jim:issue add` (ad-hoc); the 7 surfacing skills (`/jim:spec`, `/jim:research`, `/jim:plan`, `/jim:build`, `/jim:brainstorm`, `/jim:debug`, `/jim:sec`) via their end-of-phase candidate batch per spec 018; `/jim:issue list` / `stats` / `show` for read views |
