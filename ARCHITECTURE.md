...
│   │   └── scripts/
│   │       └── jimledger.sh           # build ledger + git/metric extraction
...
- **Build ledger script (`skills/review/scripts/jimledger.sh`, spec 026).** The first jim script to read git operationally (previously only `migrate.sh`'s read-only `git status` existed). `start`/`finish` record the build's baseline/head SHA; `event` appends a generic event; `metrics` emits a content-free `key=value` channel (commit counts by type, diffstat, runs/interruptions, duration); `files` lists changed paths over the validated `base..head` range. Every SHA is validated through `jimfile.sh valid-id` (the single `is_valid_id` boundary — forecloses git option-injection) before any range use; the script never `source`s ledger content and never commits (build owns the two `ledger.md` commits, at `start` and `finish`). Ledger line format is TAB-separated `<epoch>\t<iso8601>\t<phase>\t<event>\t<kv>`. Written by `/jim:build`, consumed by `/jim:review`. `set -uo pipefail; export LC_ALL=C`; tested by `tests/jimledger.sh`.
