...
spec: "docs/specs/platform/003-jimfile/spec.md"
type: feature
status: complete
---
...
9. [x] **Migrate `skills/spec/SKILL.md`** — replace the next-ID prose at L116 with `!`bash ${CLAUDE_PLUGIN_ROOT}/skills/file/scripts/jimfile.sh next-id <group>``, and replace the spec write path at L128 with `!`bash ${CLAUDE_PLUGIN_ROOT}/skills/file/scripts/jimfile.sh path spec <group> <id> <name>``. The existing specs glob at L44 already uses `jimconf.sh get specs` — leave it (different concern: directory resolution, not artifact discovery). After migration, the LLM still selects `<group>` and `<name>` from interview context; the script computes `<id>` and assembles the path.
   **Verify:** `grep -q 'jimfile.sh next-id' skills/spec/SKILL.md && grep -q 'jimfile.sh path spec' skills/spec/SKILL.md && ! grep -E 'max\(existing IDs\) ?\+ ?1' skills/spec/SKILL.md`
   ...
   > **Post-mortem (2026-05-06):** The literal `!`-injection form shown above (`path spec <group> <id> <name>`) does **not** load — Claude Code's permission parser rejects unquoted angle brackets as malformed shell redirects ("Unrecognized redirect shape"). The actual implementation in `skills/spec/SKILL.md:137-141` deviated from this template and used a fenced code block instead, which is why `/jim:spec` works. When the deferred consumers (`vision`, `roadmap`, `arch`, `plan`, `research`, `meta-skill`, `meta-agent` — see Out of Scope) are migrated, follow `skills/spec/SKILL.md`'s shipped form (fenced block, no leading `` ! ``), not the template above. Two skills (`skills/research/SKILL.md`, `skills/plan/SKILL.md`) were migrated against the broken template and required follow-up fixes. See ARCHITECTURE.md → Scripting Layer for the parser-safety constraint.
