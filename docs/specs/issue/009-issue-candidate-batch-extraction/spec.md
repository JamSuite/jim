...
- **Architect note:** Three options were already rejected — a shared reference `.md` read at runtime (permission prompt every batch step, per the `security-dod` precedent in `skills/sec`); a `Skill(jim:issue-batch)` sub-skill (the parent's `$ARGUMENTS` does not auto-forward, and serializing the accumulated candidate list as args defeats the abstraction); and refactoring `/jim:issue` into a callable subroutine (its confirm-or-edit step is LLM-prompt logic, not bash). The scripting-layer axis is viable precisely because it avoids all three. Sources: `docs/specs/issue/002-issue-tracking-workflow-integration/plan.md` (Design Decision 2), `docs/specs/issue/008-issue-pipeline-ownership/plan.md` (Design Decision 1), `ARCHITECTURE.md` → Skill Invocation / Scripting Layer.
