...
- **Affected Systems:**
  ...
  - Top-level matrix fixture: `skills/meta-matrix/` family (dispatcher + four category sub-skills — relocated by spec 014 from the original project-local fixture; rename + retain)
...
> Sentinel fixture relocated by spec 014 — see `skills/meta-matrix/` family (top-level dispatcher + four category sub-skills). The ACs below describe the matrix at 011-lock-in time; the paths reflect the post-014 relocation.

- [ ] Matrix fixture lives at `skills/meta-matrix/` family (top-level, post spec 014): a dispatcher (`skills/meta-matrix/SKILL.md`) plus four category sub-skills (`skills/meta-matrix-bash-invocation/`, `skills/meta-matrix-variable-setting/`, `skills/meta-matrix-conditional-evaluation/`, `skills/meta-matrix-skill-invocation/`). `name:` and `description:` in each SKILL.md frontmatter set; sentinel rows A–Z preserved verbatim across the sub-skills.
- [ ] `ARCHITECTURE.md` → Substitution Conventions (or a sibling subsection) references `skills/meta-matrix/` (the top-level family) as the manual regression fixture to rerun when the substitution convention is touched, with the instruction "quit and relaunch Claude Code from the repo root so the matrix skill is discovered at session start."
...
- [ ] `docs/specs/sdlc/001-meta/spec.md` line 123 (or its containing bullet) carries the same annotation. Body preserved verbatim.
...
- [ ] `docs/specs/platform/003-jimfile/spec.md` line 65 describes `get`'s return shape as path-or-`NOT_FOUND`; D8 reference flagged as superseded. `docs/specs/platform/003-jimfile/plan.md` line 61 carries a one-line trailing annotation noting the 2026-05-13 amendment.
