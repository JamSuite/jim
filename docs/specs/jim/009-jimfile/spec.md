...
- [ ] **Amended 2026-05-12 by spec 013 (D2):** `get <key>` returns the configured path *if it exists on disk*, else empty string. Existence-checking is performed inside `cmd_get` (`test -e`). Unknown-key exit-1 and malformed-invocation exit-2 are preserved. Empty-result is exit-0 so directive-vocabulary callers can no-op cleanly on an absent file (D8 of the brainstorm).
