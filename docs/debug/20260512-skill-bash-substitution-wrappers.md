...
**Further reorganized by spec 014** — sentinel rows now live under the `skills/meta-matrix-bash-invocation/` family (with conditional-evaluation rows under `skills/meta-matrix-conditional-evaluation/`, variable-setting rows under `skills/meta-matrix-variable-setting/`, and skill-invocation rows under `skills/meta-matrix-skill-invocation/`, dispatched from `skills/meta-matrix/`). Body preserved as forensic record per spec 011's `subtest`-name preservation precedent.
