...
Jim uses identifiers that require coordination to avoid collisions and merge conflicts. Every unique identity across a project — spec ordinals, group names, issue display ordinals and durable issue ids — is bound through a shared allocator. This coordination lets you work freely in git across clones, branches, and worktrees.

This document describes the mechanism and the features that consume the allocated identities.

*Note:* Jim expects a [git](https://git-scm.com/) environment. Other version control systems are not currently supported. References to clone, branch, tip, remote, refs, push, fast-forward, etc., are all in the context of git operations.
...
| **Issue ordinal** | `num:` — a positive integer, project-wide | The issue emitter, at filing time | The display handle (`#350`) |
...
Consumers: [`/jim:spec`](../../WORKFLOW.md) binds a spec ordinal before it writes anything; the [issue](issues.md) emitter binds an ordinal and a durable id for every issue; [`/jim:partition`](blueprints.md#partition-operations) binds the rename records that let a moved identity still resolve.
...
Two logs, file order authoritative: `specs.log` holds the spec and group records, `issues.log` the issue records.
...
spec  allocate cart/001 cart-spec 20260726 jim-seed
spec  rename   cart/001 checkout/001 20260727 jim-lift
...
The last two fields are the date and a provenance tag — the allocating user, or `jim-seed` / `jim-catchup` / `jim-lift` where a bootstrap or a repair wrote the record. **Provenance is advisory only.** It is self-asserted, never consulted for authorization, and never used to order anything: the log's own file order decides every outcome, not a timestamp and not a name.

### Allocation

An allocation reads the branch tip, derives the next identity from the log it just read, and creates its record with a **compare-and-swap** (CAS). Where the clone has a remote, the CAS is the push itself — git's non-fast-forward rejection is the check, so the record lands only if the tip has not moved since the read. Where a clone has no remote at all, the same discipline runs locally against the ref, coordinating that clone's own sessions and worktrees.
...
**Spent, never reclaimed.** The log only grows. Abandoning work behind an allocated identity leaves a permanent gap in the ordinals, and a refusal that fires *after* allocation still burns the allocated id. This is the cost of never issuing the same identity twice.

**Rewrites are refused, not absorbed.** Each clone remembers the registry content it last saw, in its own `.git` — never on a branch, never fetched or pushed. If the branch no longer contains that content as a prefix, the history was truncated or force-pushed, and allocation stops rather than allocating over it. A clone that has not fetched the branch yet has no baseline to compare against, so protect the coordination branch against force-push and deletion.

## Working offline

Jim's `id_coordination_unreachable` config option decides what happens when the coordination point cannot be reached — offline, no push access, a fetch that fails:

- **`fail`** (default) — no identity is issued and the failure is displayed. Nothing uncoordinated is ever written, and there is no silent local fallback.
...
The two consumers carry the provisional id differently:

- **A spec** uses it as its entire directory name — `docs/specs/core/P-20260728-widget/` — with its frontmatter claiming the same identity.
...
The registry prevents collisions only while it faithfully represents the repo. An identity created outside the allocator — by hand, or by tooling that is not Jim — never reaches the log, so the registry undercounts what the project has already used and the next allocation hands out an id something already holds. Never create Jim ids by hand; where that has already happened, the verbs below find and repair it.
...
- **Provenance is not authorization.** The provenance tag is self-asserted and advisory; nothing keys a decision on it.
...
| Key | Default | Effect |
| :--- | :--- | :--- |
| `id_coordination_mechanism` | `"git"` | How identities are coordinated; `git` uses the append-only registry described here. An unrecognized value is refused, never silently degraded |
