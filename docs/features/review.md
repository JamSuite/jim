...
A post-build review that checks the implementation against the spec and plan, and provides some basic metrics. Changes get classified, and sub-agents fan out to verify the spec's acceptance criteria, and any areas flagged by the classification. The results are judged and an alignment verdict is recorded. See [Gates and knobs](#gates-and-knobs) for configuration.

## The long version

`/jim:review` closes the loop after code lands. It verifies the build's actual implementation against the spec's acceptance criteria, the plan's tasks, and the architecture document. It measures how the build went as a process, flags security regressions in the code that landed, and records the result as a per-spec `review.md` artifact.

Two properties define its character:

- **Findings are advisory.** The review is a report, never a veto. It cannot reject a build; it tells you where the implementation diverged and lets you decide. (Whether the review *phase* is required is a separate axis — see [Gates and knobs](#gates-and-knobs).)
...
```
/jim:review docs/specs/myapp/012-rate-limiter        # review one spec's build
/jim:review --depth lean docs/specs/myapp/013-typo   # lighter pass for a trivial change
```
...
When the contract graph names the reviewed group as a provider and the build touched its provides-side code, the affected cross-group contract edges join the check: provider-side in-change violations feed the update's fork as provides-face divergences; consumer-side violations (other groups' code) are offered as issues. Results render as a Contracts subsection of the living-intent dimension with its own counts.

## Gates and knobs

Two axes that deliberately do not conflate:

- **The findings are always advisory.** No verdict blocks anything. The review never auto-rejects a build and never modifies code — it reports, like `/jim:debug` diagnoses without fixing.
- **The phase can be required.** `require_review = "true"` holds `/jim:build`'s completion gate until the review has *run to completion* — an interrupted, errored, or declined review leaves the build incomplete. It is the uncompleted phase that blocks, never the findings.
...
Review findings can also be captured as issues through the standard end-of-phase candidate batch (`issue_capture`), so drift that warrants follow-up lands in the one tracking channel instead of evaporating with the conversation.
