...
| `/jim:issue` | Capture a discovery (`add <subject>`), move one through its lifecycle (`claim` / `release` / `start` / `close` / `reopen` / `join` / `leave`), or review the collection (`list [filter]` / `stats` / `show <id>` / `insights`; bare → help); `reconcile` realizes ordinals bound while the coordination point was unreachable | `@jim:pm` | `issues/{YYYYMMDD}-{slug}.md` + `INDEX.md` |
...
- **`close <id> [--as done|wontfix|duplicate|obsolete]`** — finish it, recording how; a bare close records `done`. Any developer may close any issue, and `claimed-by` is preserved — it says who *held* the issue, not who finished it. Closing `--as duplicate` requires the superseding issue to be named in the record's `duplicates` relation. A close also appends a dated `## Resolution` to the record's body — the commit that shipped it, what pins it, and what was left alone — whatever the outcome, since that is the half the fields cannot carry.
...
- **`join <id> <umbrella>` / `leave <id> <umbrella>`** — put an issue under an umbrella, or take it out. The umbrella is named by the same reference forms `<id>` accepts. Membership is recorded on the member alone, so one record is written and the umbrella's file is untouched; an issue may belong to several umbrellas. A record that is not an epic is refused, naming what it is, and an epic may not be put under an epic. Create an umbrella with `add <subject> --type epic`, and file straight into one with `add <subject> --part-of <umbrella>`.
...
A lifecycle verb that would leave a record exactly as it found it writes nothing — no field, no timestamp, no index regeneration and nothing published — and reports `unchanged` at a success status, so grouping several issues at once is not stopped by the one already grouped.
...
**Body-lead repair (`backfill.sh heading`).** The emitter opens every issue body with a `## Description` heading, so `--body-file` holds prose only. A caller that repeats the heading files it twice; one that opens with a section of its own leaves the heading above nothing. `bash skills/issue/scripts/backfill.sh heading` removes a `## Description` whose next non-blank line is another `##` heading — collapsing a duplicated pair to one, and dropping a heading that leads no prose. A `###` beneath it is a subsection of Description rather than a sibling, and is left alone. Same preview / `--apply` / `--expect` gate as the other two. Unlike them this is not a one-time migration: the emitter still prepends unconditionally, so the shape recurs whenever a caller repeats the heading.
