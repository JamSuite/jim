...
**Duplicated-heading repair (`backfill.sh heading`).** The emitter opens every issue body with a `## Description` heading, so `--body-file` holds prose only. A caller that repeats the heading files it twice. `bash skills/issue/scripts/backfill.sh heading` collapses such a pair to one, and never removes the last heading — a record whose body opens with a section of its own keeps the heading above it, so every record converges on carrying exactly one. Same preview / `--apply` / `--expect` gate as the other two. Unlike them this is not a one-time migration: the emitter still prepends unconditionally, so the duplicate recurs whenever a caller repeats the heading.
