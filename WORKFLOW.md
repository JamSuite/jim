...
| Issue | `docs/issues/{YYYYMMDD}-{slug}.md` + `INDEX.md` (path via `issues_path`; branch via `issue_placement`) | Discovery artifacts surfaced during the workflow — one markdown file per issue with a display ordinal (`num`), `open`/`closed` status, typed relations, and an auto-generated index. By default they ride the current working branch; set `issue_placement` to a branch name to give a team one shared collection | `/jim:issue` |
...
**Checking and repairing the registry itself:** identities created outside the allocator — old habits, non-jim tooling, a partition that rewrote them — leave the registry answering below the tree, so the next allocation hands out an id the project already owns. `bash skills/file/scripts/jimalloc.sh sweep` is the read-only check: it reports tree-vs-registry drift under named classes and names what it could not cover (reserved slots, pending provisionals, groups outside coordination, rename-source ids), exiting `0` clean / `3` drift / `4` could-not-check. It has **two** repair halves, and they draw on different evidence. `catch-up` repairs an *absence* the tree can prove — preview by default, `--apply` appends the missing records as one CAS-guarded commit marked `jim-catchup`. `lift` repairs a *move the registry never witnessed*: a rename, split or merge that relocated identities before the emission verbs existed leaves no rename record, so a citation frozen beforehand dead-ends; `lift` reads the ledger's own identity-pair events and records those pairs, refusing any the registry cannot corroborate. Neither invents a winner — both refuse a contradiction and name it, because choosing between two claims is an operator decision. All three are hand-run script verbs, not skills; the in-session path is the `registry-tree-consistency` invariant on the platform blueprint, which `/jim:verify` runs as a configured operator check (`verify_command_id-sweep`).
